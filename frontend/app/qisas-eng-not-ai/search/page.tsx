// app/qisas-eng-karin/search/page.tsx
...
const chaptersDir = path.join(process.cwd(), "app/qisas-eng-karin/qisas-eng-karin-chapters");
...
function getChapterFileText(chapter: Chapter) {
  ...
  return matchingFiles.map((file) => fs.readFileSync(file, "utf8")).join(" ");
}
  ...
  const results = !cleanedQuery
    ...
    : allChapters
        .map((chapter) => {
          ...
          const matches = chapterText.match(new RegExp(cleanedQuery, "gi"));
  ...
  return (
    <div className="mx-auto max-w-2xl pb-20">
      <article className="flex justify-between">
        <div>
          <h1 className="text-lg font-semibold text-[#817a1d]">
            Search Results
          </h1>
      ...
      <div className="flex flex-col gap-3">
        {results.map((chapter, index) => (
          <Link
            key={chapter.slug}
            href={`/qisas-eng-karin/${chapter.slug}`}
            className="
              flex gap-4
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-1 text-sm text-[#8b8576]">
                This word is mentioned {chapter.matchCount} times in this
                chapter
              </div>
            </div>
          </Link>
        ...
        {query && results.length === 0 && (
          <article className="border rounded-xl px-5 pt-5 pb-10 flex flex-col items-center">
            <Image
              src={not_found}
              alt=""
              width={260}
            ...
            <p className="mt-2 text-sm lg:text-md text-center">
