...
const collectionButtons = [
{
  id: 'tawratData',
  icon: '⚖️',
  title: 'Stories from Tawrat',
  subtitle: 'The Early Prophets',
},
{
  id: 'zamindaData',
  icon: '⛰️',
  title: 'In the Promised Land',
  subtitle: 'Historical Narratives',
},
{
  id: 'xushxabarData',
  icon: '🕊️',
  title: 'Gospel stories: He is the Way',
  subtitle: 'A Journey into the Inner Life',
},
{
  id: 'safarData',
  icon: '🌾',
  title: 'Qisas of the Companions',
  subtitle: 'Following the Guidance of the Masters',
},
] satisfies {
  id: CollectionKey;
  icon: string;
...
export default function QisasUzbMainPage() {
  ...
  const STORAGE_KEY = 'qisas-eng-not-ai-selectedCollection';
  ...
  useEffect(() => {
    ...
    const savedChapter = localStorage.getItem('qisas-eng-not-ai-lastChapter');

    if (savedChapter) {
      try {
        setLastChapter(JSON.parse(savedChapter));
      } catch {
        localStorage.removeItem('qisas-eng-not-ai-lastChapter');
      }
    }
  }, []);
      ...
      <div ref={titleRef} className="scroll-mt-14">
        {lastChapter && (
          <p className="mt-6 text-[12px] leading-relaxed text-[#c00101]">
            ...
            <Link
              href={`/qisas-eng-not-ai/${lastChapter.slug}`}
          ...
          .map((chapter) => {
            ...
            return (
              <div className="course_item" key={chapter.id}>
                <Link href={`/qisas-eng-not-ai/${chapter.slug}`} className="chapter">
                ...
                <div className="sub_item">
                  {chapter.subChapters
                    ...
                    .map((sub) => (
                      <Link
                        key={sub.id}
                        href={`/qisas-eng-not-ai/${sub.slug}`}
